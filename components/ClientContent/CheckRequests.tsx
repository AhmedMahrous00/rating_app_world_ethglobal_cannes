...
export default function CheckRequests() {
  ...
  const [clientLocalNonce, setClientLocalNonce] = useState<string | null>(null);

  useEffect(() => {
    setClientLocalNonce(Math.random().toString(36).substring(2, 15));
  }, []);
  ...
  const handlePostClick = async () => {
    const response = await fetch('/api/nonce', {
      ...
      body: JSON.stringify({ localNonce: clientLocalNonce }),
    });
    const data = await response.json();
    setNonce(data.nonce);
    setLocalNonce(clientLocalNonce);
    setResponse(data);
  };
